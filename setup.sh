...
printf "%s\n" '#include <errno.h>' >> "${DestFile}"
printf "%s\n" '#include <unistd.h>' >> "${DestFile}"
printf "%s\n" '#include <dlfcn.h>' >> "${DestFile}"
printf "\n" >> "${DestFile}"
...
printf "%s\n" '#define WantInitSpeedValue 4' >> "${DestFile}"
...
printf "%s\n" '#define kAppVariationStr "minivmac-36.04-lx64"' >> "${DestFile}"
printf "%s\n" '#define kStrCopyrightYear "2018"' >> "${DestFile}"
printf "%s\n" '#define kMaintainerName "unknown"' >> "${DestFile}"
printf "%s\n" '#define kStrHomePage "(unknown)"' >> "${DestFile}"
printf "\n" >> "${DestFile}"
printf "%s\n" '#define kBldOpts "-br 36 -t lx64 -m II -hres 512 -vres 342 -depth 0 -speed 4 -mem 5M"' >> "${DestFile}"
...
printf "%s\n" '#define kRAMb_Size 0x00100000' >> "${DestFile}"
printf "\n" >> "${DestFile}"
...
DestFile="${my_config_d}SOUNDGLU.h"
printf "" > "${DestFile}"

printf "%s\n" '#include "SGLUALSA.h"' >> "${DestFile}"
...
printf "\n" >> "${DestFile}"
printf "%s\n" 'bld/OSGLUXWN.o : src/OSGLUXWN.c src/STRCNENG.h cfg/STRCONST.h src/INTLCHAR.h src/COMOSGLU.h src/CONTROLM.h src/SGLUALSA.h cfg/SOUNDGLU.h cfg/CNFGGLOB.h' >> "${DestFile}"
...
printf "%s\n" '	gcc \' >> "${DestFile}"
printf "%s\n" '		-o "minivmac" \' >> "${DestFile}"
printf "%s\n" '		$(ObjFiles) -ldl -L/usr/X11R6/lib -lX11' >> "${DestFile}"
